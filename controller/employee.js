...
// ubah pegawai
const updateEmployee = (barangBaru) => {
    const employes = loadEmployee()
    // ngilangin pegawai lama yang namanya sama dengan old nama
    const newemployes = employes.filter((employee) => employee.nama !== barangBaru.oldNama)
    delete barangBaru.oldNama
    newemployes.push(barangBaru)
    saveEmployee(newemployes)
}

const updateEmployeeById = (barangBaru, id) => {
